...
### Added
- **Azure deployment dispatch**: `docker-image.yml` now fires a `repository_dispatch` event to the private Priority-Hub-DTI repo after pushing a new image to GHCR on main, triggering the Azure CD pipeline (staging → approval → production).
